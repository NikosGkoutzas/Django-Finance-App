    ...
    def create(self , request , *args , **kwargs):
        ...
        if(all_assets):
            ...
            for card in Card.objects.filter(user_id=user_id):
                ...
                cards_balances.append(f'{card.card_type} - {card.card_number}  ->  {card.balance} {card.currency}')
            ...
            for tr in transactions:
                ...
                if(tr.type == 'Income'):
                    if(tr.payment_method == 'Cash'):
                        total_income_cash += (tr.amount if(user.currency == currency) else Currency_rate.convertion(tr.amount , user.currency , currency))
                    if(tr.payment_method == 'Card' and this_card):
                        total_income_card += (tr.amount if(this_card.currency == currency) else Currency_rate.convertion(tr.amount , this_card.currency , currency))
                    if(tr.recurring):
                        total_incomes_subscriptions += (tr.amount if(this_card.currency == currency) else Currency_rate.convertion(tr.amount , this_card.currency , currency))

                if(tr.type == 'Expense'):
                    if(tr.payment_method == 'Cash'):
                        total_expenses_cash += (tr.amount if(user.currency == currency) else Currency_rate.convertion(tr.amount , user.currency , currency))
                    if(tr.payment_method == 'Card' and this_card):
                        total_expenses_card += (tr.amount if(this_card.currency == currency) else Currency_rate.convertion(tr.amount , this_card.currency , currency))
                    if(tr.recurring):
                        total_expenses_subscriptions += (tr.amount if(this_card.currency == currency) else Currency_rate.convertion(tr.amount , this_card.currency , currency))
            ...
            for tr in transactions:
                if(tr.type == 'Income' and tr.payment_method == 'Cash'):
                    ...
                    total_expense_cash += (tr.amount if(user.currency == currency) else Currency_rate.convertion(tr.amount , user.currency , currency))
                ...
                if(chosen_card and tr.card_number == choose_card):
                    if(tr.type == 'Income'):
                        if(tr.payment_method == 'Card'):
                            total_income_card += (tr.amount if(chosen_card.currency == currency) else Currency_rate.convertion(tr.amount , chosen_card.currency , currency))
                        if(tr.recurring):
                            total_incomes_subscriptions += (tr.amount if(chosen_card.currency == currency) else Currency_rate.convertion(tr.amount , chosen_card.currency , currency))

                    if(tr.type == 'Expense'):
                        if(tr.payment_method == 'Card'):
                            total_expenses_card += (tr.amount if(chosen_card.currency == currency) else Currency_rate.convertion(tr.amount , chosen_card.currency , currency))
                        if(tr.recurring):
                            total_expenses_subscriptions += (tr.amount if(chosen_card.currency == currency) else Currency_rate.convertion(tr.amount , chosen_card.currency , currency))
