...
class Card(models.Model):
    ...
    def save(self , *args , **kwargs):
        if(not self.pk):
            ...
            expiration_date = timezone.now().today() + relativedelta(years=3)
            exp_date_month = ''
            if(len(str(expiration_date.month)) == 1):
                exp_date_month = '0' + str(expiration_date.month)
            else:
                exp_date_month = str(expiration_date.month)
            exp_date_year = str(expiration_date.year)[2:]
            expiration_date = f'{exp_date_month}' + '/' + f'{exp_date_year}'
            self.cvv = cvv
...
class Transaction(models.Model):
    ...
    subscription_last_paid_date = models.DateField(blank=True , null=True , default=timezone.now().date())
...
class Analytics(models.Model):
    ...
    expiration_date = models.CharField(max_length=10 , blank=True , null=True)
