...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    class Meta():
        model = CustomUser
        fields = ['id' , 'username', 'password' , 'job' , 'cash' , 'debt' , 'currency' , 'convert_currency']
    ...
    def validate(self , data):
        ...
        if(self.instance):
            if(not data.get('convert_currency')):
                self.instance.initial_currency = data.get('currency')

            if(data.get('convert_currency')):
                if(self.instance.initial_currency != data.get('currency')):
                    data['cash'] = Currency_rate.convertion(data.get('cash') , self.instance.initial_currency , data.get('currency'))
                    data['debt'] = Currency_rate.convertion(data.get('debt') , self.instance.initial_currency , data.get('currency'))
                    self.instance.initial_currency = data.get('currency')
                    data['convert_currency'] = True
                else:
                    data['convert_currency'] = False
    ...
    def create(self , validated_data):
        password = validated_data.pop('password') # pop, because already exists in 'AbstactUser' class
...
class CardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Card
        fields = ['id' , 'user' , 'card_type' , 'card_number' , 'cvv' , 'expiration_date' , 'balance' , 'currency' , 'convert_currency']
    ...
    def validate(self , data):
        if(self.instance):
            ...
            if(data.get('convert_currency')):
                if(self.instance.initial_currency != data.get('currency')):
                    data['balance'] = Currency_rate.convertion(data.get('balance') , self.instance.initial_currency , data.get('currency'))
                    self.instance.initial_currency = data.get('currency')
                    data['convert_currency'] = True
                else:
                    data['convert_currency'] = False
